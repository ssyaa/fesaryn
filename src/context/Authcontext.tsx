...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    const fetchCart = useCallback(async () => {
        if (!token) return;
        try {
            const res = await axios.get("http://54.253.189.135/api/cart");
    ...
    const addToCart = async (productId: number, quantity: number = 1) => {
        if (!token) return;
        try {
            await axios.post("http://54.253.189.135/api/cart", {
    ...
    const updateCartItem = async (cartItemId: number, quantity: number) => {
        if (!token) return;
        try {
            await axios.put(`http://54.253.189.135/api/cart/${cartItemId}`, {
                quantity,
            });
            await fetchCart();
        } catch (error) {
    ...
    const removeCartItem = async (cartItemId: number) => {
        if (!token) return;
        try {
            await axios.delete(`http://54.253.189.135/api/cart/${cartItemId}`);
            await fetchCart();
        } catch (error) {
    ...
    const clearCart = async () => {
        if (!token) return;
        try {
            await axios.post("http://54.253.189.135/api/cart/clear");
            setCartItems([]);
        } catch (error) {
