...
const ProductPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`http://54.253.189.135/api/products/${productId}`);
        ...
        const images = rawImages.map((img: string) =>
          img.startsWith("http") ? img : `http://54.253.189.135/storage/${img}`
        );
        ...
        const resAll = await fetch(`http://54.253.189.135/api/products`);
